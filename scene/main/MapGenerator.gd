...
func _update_neighbors(tile_pos : Vector2i, tile_name : String, tile_rule : Dictionary):
	for dir in tile_rule.keys():
		...
		if _vec2i_in_bounds(neighbor_pos) and _finalTiles[neighbor_pos.x + neighbor_pos.y * MAX_Y] == "failed":
			if _get_legal_tiles(neighbor_pos).size():
				_entropy_lookup[_get_legal_tiles(neighbor_pos).size()].erase(neighbor_pos)
			_remove_illegal_tiles(neighbor_pos, tile_rule[dir])
			if _get_legal_tiles(neighbor_pos).size():
				_entropy_lookup[_get_legal_tiles(neighbor_pos).size()].append(neighbor_pos)
...
func _generate_map():
		while not _is_empty(_entropy_lookup):
			...
			var random_tile = _get_legal_tiles(random_pos).pick_random()
			print(random_pos, random_tile)
			if random_tile:
				_finalTiles[random_pos.x + random_pos.y * MAX_Y] = random_tile
				_update_neighbors(random_pos, random_tile, TileRules.TileRule[random_tile])
